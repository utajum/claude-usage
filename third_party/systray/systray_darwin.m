...
- (void)applicationDidFinishLaunching:(NSNotification *)aNotification
{
  ...
  // PATCHED (claude-usage vendor): assign the menu directly to the status item
  // so macOS displays and positions it natively on click. The upstream code
  // instead popped the menu up manually via popUpMenuPositioningItem on each
  // click, which makes tall menus render with a scroll arrow (▲) and shift
  // their content on hover. Native positioning avoids that entirely.
  //
  // Consequence: the tappedLeft/tappedRight click callbacks (SetOnClick etc.)
  // are not invoked in this mode, since macOS owns the click. This app does not
  // use those callbacks. menuWillOpen: still fires because menu.delegate=self.
...
- (void)show_menu
{
  // PATCHED (claude-usage vendor): no-op. The menu is now assigned to the
  // status item (statusItem.menu) and shown/positioned natively by macOS on
  // click, so we must NOT also pop it up manually here — doing so brings back
  // the scroll-arrow behavior this patch removes. Kept as a stub because the
  // Go side still calls show_menu() from systray_left_click/right_click.
}
